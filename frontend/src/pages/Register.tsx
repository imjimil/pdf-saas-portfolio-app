...
export default function Register() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      setFormError(null);
      try {
        const response: AuthResponse = await authAPI.register(values.email, values.password);

        if (!response?.token || !response?.user) {
          setFormError({ message: 'The server sent back an unexpected response.' });
          toast.error('Could not create your account', 'Please try again in a moment.');
          return;
        }
        ...
        const apiError =
          error instanceof ApiError
            ? error
            : new ApiError('Could not create your account. Please try again.');
    ...
    <AppShell>
      <div className="mx-auto flex w-full max-w-md flex-col justify-center px-4 py-10 sm:px-6 sm:py-16">
        <div className="card animate-fade-up p-6 sm:p-8">
          ...
          <h1 className="mt-6 text-display-xs">Create an account</h1>
          <p className="mt-1.5 text-[15px] text-muted text-pretty">
            We store your email and a log of which tools you used — never the files themselves.
          </p>
